...
import com.poisonh.poisonh.widget.LightnessVolumeView;
...
public class PlayerGesture implements OnGestureListener
{
    ...
    public PlayerGesture(Activity activity, VideoView videoView, RelativeLayout rootView)
    {
        ...
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(DensityUtils.dp2px(activity, 60), DensityUtils.dp2px(activity, 60));
                            ...
                            float distanceY)
    {
        ...
        if (absDistanceX < absDistanceY && !intoSeek)
        { // Y方向的速率比X方向的大，即 上下 滑动
            if (distanceY > 0)
            {
                if (x1 >= width * (1 - 0.35))
                {
